From markr@eye.com Mon Mar 15 17:17:57 1993
Date: Mon, 15 Mar 93 17:17:15 -0500
From: Mark Reichert <markr@eye.com>
To: min@kodak.com, jed@graphics.cornell.edu
Subject: better than compile.el
...
This just passed thru gnu.emacs.sources. Use it, love it.
...
;;rs-compile.el|Radey Shouman|rshouman@chpc.utexas.edu|
;;more convenient compiler interface|10-93|1.0
...
;;
;;       for example:
;;(setq compile-hook 'compilation-grep-hook)
(defun compilation-grep-hook ()
  "If a grep command is being run, use only the `grep' pattern to search
for matches, open any files visited by next-error in more mode. "
  (if (string-match "/?[ef]?grep " command)
      (progn
	(compilation-regexp-list 'grep)
	(setq next-error-find-file-hook 'more-mode))))
...
;;; mcr prefers this to be nil. make your own choice.
...
(if compilation-mode-map
  ...
  (define-key compilation-mode-map "\C-c\C-g" 'compile-again)
		 ...
		 (setq command
			 ...
			 (read-string 
			  (concat (if host (concat "(" host ") "))
				  "Compile command: ")
			  (or compilation-command
			      compile-command))))
		 (list command buf)))

  (setq compile-command command)
  (setq compile-last-compilation-buffer buf)
  (compile1 compile-command "error" nil buf 'compile))
...
but wait, there's MORE ... "
  ...
  (let ((cwd default-directory)         ; catch that slippery animal
    ...
    (make-local-variable 'compilation-command)
    ...
    (setq compilation-command command
	  ...
	  compilation-regexp-list (default-value 'compilation-regexp-list))
	  ...
	  (cond ((null (buffer-name buf))
		...
		((memq (process-status process) '(signal exit))
		 ...
		 (let* ((data (match-data))
		   ...
		   (cond ((= n 0)
			  (insert "  no " compilation-error-type "s found."))
			 ((= n 1)
			  (insert "  one " compilation-error-type " found."))
			 (t (insert (format "  %d %ss found." n
	...
	(and move-point
      ...
      (split-window (get-largest-window)
		    (- (window-height)
			 ...
			 5)))
      (set-window-buffer (get-lru-window) buf)
      (set-window-point (get-buffer-window buf) (point-max)))))
...
(defun compilation-parse-line ()
  (let ((parse-list compilation-regexp-list)
	...
	(let ((rule-list (car parse-list)))
      ...
      (and filename linenum (list filename linenum filename-2 linenum-2)))))
	...
	(if filename
	    (let ((error-marker (compilation-get-marker filename linenumber)))
		  ...
		  (progn
			...
			n(switch-to-buffer (marker-buffer error-marker))
		      (pop-to-buffer (get-buffer compilation-last-error-buf))
		      ...
		      (save-excursion
			(beginning-of-line (- 2 (/ (window-height) 2)))
...
(defun compilation-read-dir-name-internal (string dir action)
  (if (eq action 'lambda)
    ...
    (let* ((name (file-name-nondirectory string))
      ...
      (setq possible (file-name-all-completions name realdir))
      (while possible
	(if (string= (file-name-directory (car possible)) (car possible))
	    (setq accepted 
		  (cons (car possible) accepted)))
	(setq possible (cdr possible)))
      (if action
	  accepted
	(if (null accepted)
	    nil
