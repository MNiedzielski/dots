...
;; is ugly).
(setq x--mouse-ignore 'x--mouse-ignore)
...
this height."
  ...
  (select-window x-mouse-win-0)))
(setq x-mouse-cms2-window-click 'x-mouse-split-vertically)
...
this position from the left."
  ...
  (select-window x-mouse-win-0)))
(setq x-mouse-cms2-window-click 'x-mouse-split-horizontally)
...
(defun x-mouse-copy-thing ()
  "Uses the thing package to copy the thing under the point.
The thing is either a word or sexp.  You must be at the beginning
of the sexp for this to work."
  (select-window x-mouse-win-u)
...
If menus are unavailable, it'll put the text in a *Help* buffer."
  (let* ((menubase
	 ...
	 (text '(lambda (var)
		  ...
		  ;; for var of type "x-mouse-c1-window-click".  If the
		  ;; function is a lambda expression, it returns
		  ;; "Lambda expression." instead.
		  (let* ((func (symbol-value (intern-soft var)))
			 (doc  (if (documentation func)
				   (documentation func)
				 "Lambda expression."))
			 (i 0)
			 (done nil))
	 ...
	 (line '(lambda (func mods but where type)
		  ...
		  (let* ((but-var  (concat "x-mouse-" mods but "-"
					   where "-" type))
			 (but-func (symbol-value (intern-soft but-var)))
			 spaces txt)
		    (if (eq but-func func) ()
