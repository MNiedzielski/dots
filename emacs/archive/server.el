...
(defun server-process-filter (proc string)
  ...
  (if (not (eq 0 (string-match "Client: " string)))
	...
	(setq string (substring string (match-end 0))))
      ...
      ;;
      ;; following blob added by mcr 88/6/2 for use with emacstool.
      ;; should open frame when new files come in.
      ;;
      ( send-string-to-terminal "\e[1t" )
...
as a suggestion for what to select next."
  (let ((running (eq (process-status server-process) 'run))
    ...
    ;;
    ;; I don't like my buffers buried
    ;; (bury-buffer buffer)
    ;;next-buffer
    nil
    )
