...
;;; 21-Jul-1993|1.2|~/misc/multi-click.el.Z|
...
skip back and set mark"
  (let ((syntax (char-to-string (char-syntax (char-after posn)))))
...
if MODE is 1 then set point to start of word at (min START END), mark to end of
word at (max START END)
...
If point was set with a triple click then set mark on whole line"
  ...
  (let ((click-posn (posn-point (event-start click))))
    (if (and (eq last-command 'kill-region)
        ...
        (progn
          (let ((buffer-undo-listbuffer-undo-list t))
